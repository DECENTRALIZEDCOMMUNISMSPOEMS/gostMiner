...
static int opt_needforsleep=15;
char *opt_cert;
char *opt_proxy;
long opt_proxy_type;
...
"  -n, --needforsleep=N number for retry (default: 15)\n"
...
static void parse_arg (int key, char *arg)
{
	...
	case 'n':
		v=atoi(arg);
		if(v < 1 || v > 9999) show_usage_and_exit(1);
		opt_needforsleep=v;
		break;
	case 's':
		v = atoi(arg);
